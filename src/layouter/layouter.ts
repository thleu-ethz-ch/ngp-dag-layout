...
export default abstract class Layouter {
    ...
    public async layout(renderGraph: RenderGraph): Promise<LayoutGraph> {
        ...
        this._removeCycles(layoutGraph);
        ...
        this._restoreCycles(layoutGraph);
        this._placeLoops(layoutGraph);
        ...
        _.forEach(graph.allNodes(), (node: LayoutNode) => {
            ...
            if (node.inConnectors.length > 0) {
                ...
                _.forEach(arrangedInConnectors, (connector: LayoutConnector, i) => {
                    let connectorX = firstX + (CONNECTOR_SIZE + CONNECTOR_SPACING) * i;
                    let connectorY = y;
                    if (connector.width === 0) {
                        connectorX += CONNECTOR_SIZE / 2;
                        connectorY += CONNECTOR_SIZE / 2;
                    }
                    connector.setPosition(connectorX, connectorY);
                });
            }
    ...
    private createLayoutGraph(renderGraph: RenderGraph): LayoutGraph {
        const transformSubgraph = (renderGraph: RenderGraph): LayoutGraph => {
            ...
            _.forEach(renderGraph.edges(), (edge: RenderEdge) => {
                ...
                // add dummy connectors (with name=null)
                ...
                edge.layoutEdge = new LayoutEdge(srcLayoutNode.id, dstLayoutNode.id, edge.srcConnector, edge.dstConnector);
    ...
    private _placeLoops(graph: LayoutGraph) {
        _.forEach(graph.allNodes(), (node: LayoutNode) => {
            if (node.selfLoop !== null) {
                ...
                node.width -= this._options.targetEdgeLength;
            }
        });
    }
}
