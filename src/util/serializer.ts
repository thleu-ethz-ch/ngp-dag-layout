export default class Serializer {
    ...
    static async serializePromises(promiseFunctions: Array<() => Promise<any>>): Promise<Array<any>> {
        const results = [];
        ...
        for (let p = 0; p < promiseFunctions.length; ++p) {
            results.push(await createPromise(promiseFunctions[p]));
        }
        return results;
    }
}
